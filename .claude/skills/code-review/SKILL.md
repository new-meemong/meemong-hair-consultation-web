...
리뷰 시간의 대부분은 툴 실행이 아니라 모델 왕복이다(2026-09-04 실측: 툴 7%, 왕복 93%).

- **턴당 툴 호출 1개는 금지다.** 서로 의존하지 않는 조회는 반드시 한 메시지에 묶는다.
- 심볼 위치를 모를 때 "위치 찾기 → 읽기"로 나누지 않는다. `grep -rn "심볼" -A 8 <dir>` 한 번으로 끝낸다.
- 린트·테스트처럼 초 단위가 걸리는 명령은 리뷰 시작 직후 `run_in_background`로 먼저 띄우고, 읽기를 계속한다.
